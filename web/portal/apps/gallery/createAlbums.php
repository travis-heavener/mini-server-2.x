<?php
    ...
    function get_user_albums($user_id, $key) {
        ...
        $statement = $mysqli->prepare(
            "SELECT `id`, `name`, `album_name`, `MIME`, `vector` FROM `gallery` WHERE `created` IN (SELECT MAX(`created`) FROM `gallery` WHERE `user_id`=? GROUP BY `album_name`) ORDER BY `created` DESC"
        );
